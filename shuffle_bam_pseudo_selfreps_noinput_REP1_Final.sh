...
# Make Directories
#mkdir -p /mnt/isilon/maris_lab/target_nbl_ngs/AtacSeqAnalysis/revision/IDR/Pseduo_Self_Rep_IDR/macs
#mkdir -p /mnt/isilon/maris_lab/target_nbl_ngs/AtacSeqAnalysis/revision/IDR/Pseduo_Self_Rep_IDR/pooled_pseudoreps
...
macsDir=/mnt/isilon/maris_lab/target_nbl_ngs/AtacSeqAnalysis/revision/IDR/Pseduo_Self_Rep_IDR/macs
outputDir=/mnt/isilon/maris_lab/target_nbl_ngs/AtacSeqAnalysis/revision/IDR/Pseduo_Self_Rep_IDR/pooled_pseudoreps
...
cat ${tmpDir}/${EXPT}_REP1_header.sam ${tmpDir}/${EXPT}_REP1_00 | samtools view -bS - > ${outputDir}/${EXPT}_REP1_00.bam
cat ${tmpDir}/${EXPT}_REP1_header.sam ${tmpDir}/${EXPT}_REP1_01 | samtools view -bS - > ${outputDir}/${EXPT}_REP1_01.bam
...
samtools merge -u ${outputDir}/${NAME1}_merged_pseudo.bam ${outputDir}/${NAME1}_Self_REP1_00.bam ${outputDir}/${NAME1}_Self_REP1_01.bam

#Peak calling on pseudoreplicates
echo "Calling peaks for pseudoreplicate1"
macs2 callpeak -t ${outputDir}/${NAME1}_Self_REP1_00.bam -f BAMPE -g hs -n $macsDir/${NAME1}_00_pr --nomodel -B -p 1e-3 --verbose 3 --extsize 200 --shift -100 --SPMR > $macsDir/${NAME1}_00_pr_macs2.log
...
macs2 callpeak -t ${outputDir}/${NAME1}_Self_REP1_01.bam -f BAMPE -g hs -n $macsDir/${NAME1}_01_pr --nomodel -B -p 1e-3 --verbose 3 --extsize 200 --shift -100 --SPMR > $macsDir/${NAME1}_01_pr_macs2.log
...
macs2 callpeak -t ${outputDir}/${NAME1}_merged_pseudo.bam -f BAMPE -g hs -n $macsDir/${NAME1}_merged_self_pr --nomodel -B -p 1e-3 --verbose 3 --extsize 200 --shift -100 --SPMR > $macsDir/${NAME1}_pr_merged_macs2.log

#Sort peak by -log10(p-value)
echo "Sorting peaks..."
sort -k8,8nr $macsDir/${NAME1}_00_pr_peaks.narrowPeak > $macsDir/${NAME1}_00_pr_sorted.narrowPeak
sort -k8,8nr $macsDir/${NAME1}_01_pr_peaks.narrowPeak > $macsDir/${NAME1}_01_pr_sorted.narrowPeak
sort -k8,8nr $macsDir/${NAME1}_01_pr_peaks.narrowPeak > $macsDir/${NAME1}_merged_self_pr.narrowPeak
...
idr --samples $macsDir/${NAME1}_00_pr_sorted.narrowPeak $macsDir/${NAME1}_01_pr_sorted.narrowPeak --peak-list $macsDir/${NAME1}_merged_self_pr.narrowPeak --input-file-type narrowPeak --output-file ${EXPT}_REP1_self_pseudo-idr --rank p.value --plot
